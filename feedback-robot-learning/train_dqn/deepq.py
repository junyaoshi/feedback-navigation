...
from train_dqn.replay_buffer import ReplayBuffer, PrioritizedReplayBuffer
from train_dqn.utils import ObservationInput
from train_dqn.models import build_q_func, build_hr_func
from train_dqn import build_train
        ...
        for t in range(total_timesteps):
            ...
            if use_feedback:
                if use_real_feedback:
                    feedbacks, action_idxs = get_feedback_from_LSL(feedback_LSL_stream)
                feedback_epi_buffer = [state_action_buffer[action_idx_buffer.index(a_idx)] + [feedback]
                                       for a_idx, feedback in zip(action_idxs, feedbacks)]

                # add feedbacks into feedback replay buffer
                if feedback_epi_buffer:
                    epi_feedback_test_num += len(feedback_epi_buffer) * (1 - feedback_training_prop)
                    epi_test_int = int(epi_feedback_test_num)
                    epi_feedback_test_num -= epi_test_int
                    epi_test_inds = np.random.choice(len(feedback_epi_buffer), epi_test_int, replace=False)
                    epi_train_inds = [ind for ind in range(len(feedback_epi_buffer)) if ind not in epi_test_inds]
                    feedback_buffer_train.extend([feedback_epi_buffer[ind] for ind in epi_train_inds])
                    feedback_buffer_valid.extend([feedback_epi_buffer[ind] for ind in epi_test_inds])
            ...
            if t > learning_starts and t % train_freq == 0:
                ...
                for _ in range(dqn_epochs):
                    ...
                    td_errors = train_rl(obses_t, actions, rewards, obses_tp1, dones, weights)
                    if prioritized_replay:
                        new_priorities = np.abs(td_errors) + prioritized_replay_eps
                        replay_buffer.update_priorities(batch_idxes, new_priorities)
            
            # train feedback regressor
            if use_feedback and len(feedback_buffer_train) >= min_feedback_buffer_size and t <= only_use_hr_until:
                for i in range(feedback_epochs):

                    if i < feedback_epochs * feedback_training_new_prop:
                        inds = np.arange(len(feedback_buffer_train) - feedback_batch_size, len(feedback_buffer_train))
                    else:
                        inds = np.random.choice(len(feedback_buffer_train), feedback_batch_size, replace=False)

                    np.random.shuffle(inds)
                    for start in range(0, feedback_batch_size, feedback_minibatch_size):
                        end = start + feedback_minibatch_size
                        obses = np.asarray([feedback_buffer_train[idx][0] for idx in inds[start:end]])
                        actions = np.asarray([feedback_buffer_train[idx][1] for idx in inds[start:end]])
                        feedbacks = np.asarray([feedback_buffer_train[idx][2] for idx in inds[start:end]])
                        pred, loss = train_hr(obses, actions, feedbacks)

                obs_train       = np.asarray([feedback[0] for feedback in feedback_buffer_train])
                actions_train   = np.asarray([feedback[1] for feedback in feedback_buffer_train])
                feedbacks_train = np.asarray([feedback[2] for feedback in feedback_buffer_train])
                obs_valid       = np.asarray([feedback[0] for feedback in feedback_buffer_valid])
                actions_valid   = np.asarray([feedback[1] for feedback in feedback_buffer_valid])
                feedbacks_valid = np.asarray([feedback[2] for feedback in feedback_buffer_valid])
                train_acc, train_loss = evaluate_hr(obs_train, actions_train, feedbacks_train)
                valid_acc, valid_loss = evaluate_hr(obs_valid, actions_valid, feedbacks_valid)
                print("HR: train acc {:>4.2f}, loss {:>5.2f}; valid acc {:>4.2f}, loss {:>5.2f}".format(
