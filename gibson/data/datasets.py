...
def get_item_fn(inds, select, root, loader, transform, off_3d, target_transform, depth_trans, off_pc_render, dll, train,
    ...
    paths = (
    [os.path.join(root, scene, 'pano', 'rgb', "point_" + item + "_view_equirectangular_domain_rgb.png") for item in
     uuids])
    mist_paths = (
    [os.path.join(root, scene, 'pano', 'mist', "point_" + item + "_view_equirectangular_domain_mist.png") for item in
     uuids])
    normal_paths = (
    [os.path.join(root, scene, 'pano', 'normal', "point_" + item + "_view_equirectangular_domain_normal.png") for item
     in uuids])
    pose_paths = ([os.path.join(root, scene, 'pano', 'points', "point_" + item + ".json") for item in uuids])
    semantic_paths = (
    [os.path.join(root, scene, 'pano', 'semantic', "point_" + item + "_view_equirectangular_domain_semantic.png") for
     item in uuids])
    poses = []
...
class ViewDataSet3D(data.Dataset):
    def __init__(self, root=None, train=False, transform=None, mist_transform=None, loader=default_loader, seqlen=5,
        ...
        self._require_rgb = "RGB_FILLED" in self.env.config["ui_components"] or "RGB_PREFILLED" in self.env.config[
            "ui_components"] or "rgb_filled" in self.env.config["output"] or "rgb_prefill" in self.env.config["output"]
        self.off_3d = off_3d
        self.select = []
        ...
        # Try to load start and stop waypoints
        self.navigation_scenarios_root = os.path.join(os.path.dirname(os.path.abspath(assets.__file__)),
                                                      "navigation_scenarios", 'waypoints')
        if os.path.isdir(self.navigation_scenarios_root) and only_load is not None:
            waypoints_json = os.path.join(self.navigation_scenarios_root, '{}.json'.format(only_load))
            if os.path.isfile(waypoints_json):
                waypoints = json.load(open(waypoints_json, 'r'))
                self.start_position = [waypoints[0]["startX"], waypoints[0]["startY"], waypoints[0]["startZ"]]
                ...
                with open(posefile) as f:
                    for line in f:
                        ...
                        if os.path.isfile(os.path.join(self.root, scene, 'pano', 'points', 'point_' + uuid + '.json')):
                            if np.linalg.norm(
                                    np.array(xyz) - np.array([0, 0, 0])) > 1e-5:  # remove scans that are not registered
    ...
    def __getitem__(self, index):
        ...
        paths = (
            [os.path.join(self.root, scene, 'pano', 'rgb', "point_" + item + "_view_equirectangular_domain_rgb.png") for
             item in uuids])
        mist_paths = (
            [os.path.join(self.root, scene, 'pano', 'mist', "point_" + item + "_view_equirectangular_domain_mist.png")
             for
             item in uuids])
        normal_paths = (
    ...
    def get_multi_index(self, uuids):
        ...
        p = Pool(16)
        partial_fn = partial(get_item_fn, select=self.select, root=self.root, loader=self.loader,
                             transform=self.transform, off_3d=self.off_3d, target_transform=self.target_transform,
                             depth_trans=self.depth_trans, off_pc_render=self.off_pc_render, dll=self.dll,
                             train=self.train, require_rgb=self._require_rgb)
