...
class BaseRobotEnv(BaseEnv):
    ...
    def __init__(self, config, tracking_camera, scene_type="building", gpu_idx=0, start_port=5556):
        BaseEnv.__init__(self, config, scene_type, tracking_camera)
        print("Finished setting up BaseEnv")
    ...
    def robot_introduce(self, robot):
        ...
        self.window_width = self.config["window_width"] if "window_width" in self.config else self.robot.resolution
        self.window_height = self.config["window_height"] if "window_height" in self.config else self.robot.resolution
        # TODO: We can fix scale_up later
        self.scale_up = 1  # int(512 / self.windowsz)

        # if "fast_lq_render" in self.config and self.config["fast_lq_render"] == True:
        #     self.scale_up *= 2
        # if fast render, use lower quality point cloud
        self._render_width = self.window_width
        self._render_height = self.window_height
...
class CameraRobotEnv(BaseRobotEnv):
    ...
    def __init__(self, config, gpu_idx, scene_type, tracking_camera, start_port=5556, use_filler=None):
        # The following properties are already instantiated inside xxx_env.py:
        BaseRobotEnv.__init__(self, config, tracking_camera, scene_type, gpu_idx, start_port=start_port)
        print("Finished setting up BaseRobotEnv")
    ...
    def reset_observations(self):
        ...
        self.render_rgb_filled = np.zeros((self.window_width, self.window_height, 3))
        self.render_rgb_prefilled = np.zeros((self.window_width, self.window_height, 3))
        self.render_depth = np.zeros((self.window_width, self.window_height, 1))
        self.render_normal = np.zeros((self.window_width, self.window_height, 3))
        self.render_semantics = np.zeros((self.window_width, self.window_height, 3))
    ...
    def setup_rendering_camera(self):
        ...
        if self.config["display_ui"]:
            ...
            self.UI = ui_map[ui_num](window_width=self.window_width, window_height=self.window_height, env=self,
                                     port=self.port_ui)
        ...
        if self.config["mode"] == "web_ui":
            ui_num = self.config["ui_num"]
            self.webUI = ui_map[ui_num](window_width=self.window_width, window_height=self.window_height, env=self,
                                        port=self.port_ui, use_pygame=False)
    ...
    def _close(self):
        ...
        if self._require_camera_input:
            if self.r_camera_mul:
                self.r_camera_mul.terminate()
            if self.r_camera_rgb:
    ...
    def setup_camera_multi(self):
        ...
        render_semt = "./depth_render --GPU {} --modelpath {} -t 1 -r {} -c {} -w {} -h {} -f {} -p {}".format(
            self.gpu_idx, self.model_path, self._semantic_source, self._semantic_color, self.window_width,
            self.window_height,
...
class SemanticRobotEnv(CameraRobotEnv):
    ...
    def setup_semantic_parser(self):
        ...
        if self._semantic_source == 2:
            ...
            _, semantic_ids, _ = get_segmentId_by_name_2D3DS(osp.join(self.model_path, "semantic.mtl"),
                                                             osp.join(self.model_path, "semantic.obj"), "chair")
