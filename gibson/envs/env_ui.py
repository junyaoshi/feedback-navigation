...
class SimpleUI:
    ...
    def __init__(self, width_col, height_col, window_width, port, env=None, save_first=True, use_pygame=True, window_height=None):
        if window_height is None:
            window_height = window_width

        self.env = env
        self.width = width_col * window_width
        self.height = height_col * window_height
    ...
    def _add_all_images(self):
        for index, component in enumerate(self.components):
            img = np.zeros((self.window_width, self.window_height, 3))
    ...
    def update_view(self, view, tag):
        ...
        if self.save_first and self.nframe <= len(self.POS):
            import scipy.misc
            img = np.zeros((view.shape[0], view.shape[1], 3), dtype=np.uint8)
            img[:, :, :] = view
    ...
    def _add_image(self, img, x, y):
        ...
        domain = self.screen_arr[x: x + img.shape[0], y:y + img.shape[1], :].shape
        self.screen_arr[x: x + img.shape[0], y:y + img.shape[1], :] = img[0:domain[0], 0:domain[1], :]
...
class OneViewUI(SimpleUI):
    ...
    def __init__(self, window_width=256, window_height=None, env=None, port=-1, use_pygame=True):
        if window_height is None:
            window_height = window_width
        self.POS = [
            (0, 0)  # One
        ]
        SimpleUI.__init__(self, 1, 1, window_width, port, env, use_pygame=use_pygame, window_height=window_height)
...
class TwoViewUI(SimpleUI):
    ...
    def __init__(self, window_width=256, window_height=None, env=None, port=-1, use_pygame=True):
        ...
        SimpleUI.__init__(self, 2, 1, window_width, port, env, use_pygame=use_pygame, window_height=window_height)
...
class ThreeViewUI(SimpleUI):
    ...
    def __init__(self, window_width=256, window_height=None, env=None, port=-1, use_pygame=True):
        ...
        self.POS = [
            (0, 0),  # One
            (window_width, 0),  # Two
            (window_width * 2, 0)  # Three
        ]
        SimpleUI.__init__(self, 3, 1, window_width, port, env, use_pygame=use_pygame, window_height=window_height)
...
class FourViewUI(SimpleUI):
    ...
    def __init__(self, window_width=256, window_height=None, env=None, port=-1, use_pygame=True):
        ...
        self.POS = [
            (0, 0),  # One
            (0, window_height),  # Two
            (window_width, 0),  # Three
            (window_width, window_height)  # Four
        ]
        SimpleUI.__init__(self, 2, 2, window_width, port, env, use_pygame=use_pygame, window_height=window_height)
