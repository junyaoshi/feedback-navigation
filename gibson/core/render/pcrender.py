...
class PCRenderer:
    ...
    def __init__(self, port, imgs, depths, target, target_poses, scale_up, semantics=None,
                 gui=True, use_filler=True, gpu_idx=0, window_width=256, window_height=None, env=None):
        ...
        self.show = np.zeros((self.window_width, self.window_height, 3), dtype='uint8')
        self.show_rgb = np.zeros((self.window_width, self.window_height, 3), dtype='uint8')
        self.show_semantics = np.zeros((self.window_width, self.window_height, 3), dtype='uint8')

        self.show_prefilled = np.zeros((self.window_width, self.window_height, 3), dtype='uint8')
        self.surface_normal = np.zeros((self.window_width, self.window_height, 3), dtype='uint8')
        ...
        self.imgv = Variable(torch.zeros(1, 3, self.window_height, self.window_width), volatile=True).cuda()
        self.maskv = Variable(torch.zeros(1, 2, self.window_height, self.window_width), volatile=True).cuda()
        self.mean = torch.from_numpy(np.array([0.57441127, 0.54226291, 0.50356019]).astype(np.float32))
        self.mean = self.mean.view(3, 1, 1).repeat(1, self.window_height, self.window_width)
    ...
    def render(self, rgbs, depths, pose, model, poses, target_pose, show, show_prefilled=None, is_rgb=False):
        ...
        wo, ho = self.window_width * 4, self.window_height * 3
        ...
        if pano:
            ...
            opengl_arr = np.frombuffer(mist_msg, dtype=np.float32).reshape((self.window_height, self.window_width))
            # smooth_arr = np.frombuffer(dept_msg, dtype=np.float32).reshape((n, n))
            if self._require_normal:
                normal_arr = np.frombuffer(norm_msg, dtype=np.float32).reshape((self.window_height, self.window_width, 3))
            if self._require_semantics:
                semantic_arr = np.frombuffer(semt_msg, dtype=np.uint32).reshape((self.window_height, self.window_width, 3))
        ...
        def _render_pc(opengl_arr, imgs_pc, show_pc):
            ...
            cuda_pc.render(ct.c_int(len(imgs_pc)),
                           ...
                           ct.c_int(self.window_height),
                           ct.c_int(self.window_width),
        ...
        if is_rgb:
            ...
            # Store prefilled rgb
            show_prefilled[:] = show.reshape(show_prefilled.shape)
        ...
        if self._require_semantics:
            ...
            debugmode = 0
            if debugmode:
                print("Semantics array", np.max(semantic_arr), np.min(semantic_arr), np.mean(semantic_arr),
                      semantic_arr.shape)
    ...
    def renderOffScreen(self, pose, k_views=None, rgb=True):
        ...
        # moved the reshape before self.render is called
        self.show = np.reshape(self.show, (self.window_height, self.window_width, 3))
        self.render(self.imgs_topk, self.depths_topk, self.render_cpose.astype(np.float32), self.model,
                    self.relative_poses_topk, self.target_poses[0], self.show, self.show_prefilled, is_rgb=rgb)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--models', type=str, default='', help='path of models')

    opt = parser.parse_args()
    d = ViewDataSet3D(root=opt.datapath, transform=np.array, mist_transform=np.array, seqlen=2, off_3d=False,
                      train=True)
