...
class BaseRobot:
    ...
    def __init__(self, model_file, robot_name, scale=1, env=None):
        ...
        try:
            ...
            self.eyes = self.parts["head_camera_depth_frame"]
    ...
    def reset(self):
        if self.robot_ids is None:
            self._load_model()
        if len(self.config["initial_orn"]) is 3:
            self.robot_body.reset_orientation(quatToXYZW(euler2quat(*self.config["initial_orn"]), 'wxyz'))
        else:
            self.robot_body.reset_orientation(quatToXYZW(self.config["initial_orn"], 'wxyz'))
...
class BodyPart:
    def __init__(self, body_name, bodies, body_index, body_part_index, scale, model_type):
        ...
        self.bodyIndex = body_index
        self.bodyPartIndex = body_part_index
        if model_type == "MJCF":
            self.scale = scale
        else:
            self.scale = 1
